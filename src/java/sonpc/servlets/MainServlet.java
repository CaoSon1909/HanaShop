...
public class MainServlet extends HttpServlet {
    ...
    private final String SEARCH_CUSTOMER_DYNAMIC_PAGE = "customerSearch.jsp";
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = SEARCH_CUSTOMER_DYNAMIC_PAGE;
        try {
            //load all food from DB
            TblFoodDAO foodDAO = new TblFoodDAO();
            List<TblFoodDTO> list = foodDAO.getAllActivatedFood();
            if (list != null){
                request.setAttribute("FOOD_LIST", list);
            }
            //load food category list from DB
            TblFoodCategoryDAO foodCategoryDAO = new TblFoodCategoryDAO();
            List<TblFoodCategoryDTO> category = foodCategoryDAO.loadFoodCategory();
            if (category != null){
