...
public class ShowHistoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = "";
        try {
            //load toàn bộ order từ tbl_Order lên. setAttribute vào reqScope rồi forward sang history.jsp
            //khi nhấn detail(link) sẽ hiển thị order detail (ở trang order.jsp)
            HttpSession session = request.getSession(false);
            if (session != null) {
                TblUserDTO user = (TblUserDTO) session.getAttribute("USER_DTO");
                if (user != null) {
                    TblOrderDAO dao = new TblOrderDAO();
                    List<TblOrderDTO> list = dao.getOrderListByEmail(user.getEmail());
                    if (list != null) {
